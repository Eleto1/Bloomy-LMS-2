...
import { 
  ...
  Paperclip, ArrowLeft, Star, LayoutDashboard, Link as LinkIcon, ClipboardList, Check, CalendarDays 
} from 'lucide-react';
...
export default function StudentCourseViewer() {
  ...
  // Calculate Module Progress (Ignore Headers)
  ...
  // Check if Module is locked by Date or Previous Module
  const isModuleUnlocked = (moduleIndex: number) => {
    const mod = modules[moduleIndex];
    // 1. Date Lock
    if (mod.unlock_date && new Date(mod.unlock_date) > new Date()) return false;
    // 2. Previous Module Completion
    if (moduleIndex === 0) return true;
    const prevModule = modules[moduleIndex - 1];
    return getModuleProgress(prevModule.id) === 100;
  };

  // Check if Lesson is locked by previous lesson completion
  const isLessonUnlocked = (lesson: Lesson) => {
    ...
    if (idx === 0) return true; // First lesson in module
    return progress[moduleLessons[idx-1].id]; // Previous done?
  };
  ...
  const openLesson = (lesson: Lesson, name: string) => {
    setActiveLesson(lesson); setActiveModuleName(name); 
    setCurrentQIndex(0); setSelectedAnswer(null); setQuizScore(0); setSurveyInputs({}); 
    setViewMode('lesson');
  };
  ...
  const handleQuizSubmit = async () => {
    ...
    if (isCorrect) { setQuizScore(prev => prev + 1); toast({ title: '✅ Correct!' }); } 
    else toast({ title: '❌ Incorrect', variant: 'destructive' });

    if (currentQIndex < activeLesson.quiz_data.length - 1) {
      setCurrentQIndex(prev => prev + 1); setSelectedAnswer(null);
    } else {
      ...
      if (passed) {
        ...
        setCurrentQIndex(0); setQuizScore(0); setSelectedAnswer(null);
      }
    }
  };
  ...
  const handleSurveySubmit = async () => {
    ...
    if (ratingQ && !surveyInputs[ratingQ.q]) { toast({ title: 'Please provide a rating', variant: 'destructive' }); return; }

    // Save Response (Fix: convert empty string to null for topic_id)
    const { error } = await supabase.from('survey_responses').insert({
      lesson_id: activeLesson.id, 
      topic_id: activeLesson.content || null, 
      user_id: user.id,
      rating: ratingQ ? surveyInputs[ratingQ.q] : 0, 
      feedback: surveyInputs['feedback'] || '', 
      answers: surveyInputs
    });

    if (error) {
      if (error.code === '23505') toast({ title: 'Already Submitted', variant: 'destructive' });
  ...
  if (!authorized) return <div className="p-10 text-center"><h2 className="text-xl font-bold">Access Denied</h2><Button variant="outline" className="mt-4" onClick={() => window.history.back()}>Go Back</Button></div>;
      ...
      {viewMode === 'list' && (
        <div className="max-w-3xl mx-auto space-y-6">
          <div className="flex items-center gap-2 text-sm text-gray-500">
            <Link to="/student/courses">My Courses</Link><span>/</span>
                ...
                <Card key={mod.id} className={`overflow-hidden ${!unlocked ? 'opacity-60' : ''}`}>
                  ...
                  <div className="p-2 space-y-1">
                    {lessons.filter(l => l.module_id === mod.id).map(les => {
                      ...
                      return (
                        <div key={les.id} onClick={() => open && openLesson(les, mod.title)} 
                          className={`flex items-center justify-between p-3 rounded border cursor-pointer ${done ? 'bg-green-50 border-green-200' : open ? 'bg-white hover:bg-gray-50' : 'bg-gray-50 cursor-not-allowed'}`}>
            ...
            {activeLesson.file_url && activeLesson.type !== 'header' && (
              <div className="mb-6 p-4 border rounded-lg bg-blue-50 border-blue-200">
                ...
                <a href={activeLesson.file_url} target="_blank" rel="noreferrer">
                  <Button variant="outline" size="sm"><Paperclip className="w-4 h-4 mr-2" /> Download File</Button>
                </a>
              </div>
            )}
            ...
            {activeLesson.type === 'quiz' && activeLesson.quiz_data && (
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  {activeLesson.quiz_data[currentQIndex]?.a.map((ans: string, i: number) => (
                    <div key={i} onClick={() => setSelectedAnswer(i)} 
                      className={`p-3 border rounded cursor-pointer ${selectedAnswer === i ? 'border-blue-500 bg-blue-50' : ''}`}>
                      {ans}
                    </div>
                  ))}
                </div>
            ...
            {activeLesson.type === 'survey' && activeLesson.quiz_data && (
              <div className="space-y-6">
                  ...
                  <div key={idx} className="space-y-2 border-t pt-4">
                    ...
                    {q.type === 'rating' && (
                      <div className="flex gap-1 mt-2">
                        {[1,2,3,4,5].map(star => (
                          <button key={star} onClick={() => setSurveyInputs(p => ({...p, [q.q]: star}))} 
                            className={`${(surveyInputs[q.q]||0) >= star ? 'text-yellow-400' : 'text-gray-300'}`}>
                    ...
                    {q.type === 'multiple_choice' && (
                      <div className="grid grid-cols-2 gap-2 mt-2">
                        {q.a.map((opt: string, i: number) => (
                          <div key={i} onClick={() => setSurveyInputs(p => ({...p, [q.q]: i}))}
                            className={`p-2 border rounded cursor-pointer text-sm ${surveyInputs[q.q] === i ? 'border-blue-500 bg-blue-50' : ''}`}>
                            {opt}
                          </div>
                        ))}
                      </div>
                ...
                <div className="border-t pt-4">
                  ...
                  <Textarea value={surveyInputs['feedback'] || ''} onChange={e => setSurveyInputs(p => ({...p, ['feedback']: e.target.value}))} />
                </div>
